...
function stop {
    echo "Executing command: 'docker compose down' with all profiles and latest images"
    docker compose --env-file .env.dev --profile emr --profile bahmni-lite --profile bahmni-standard --profile bahmni-mart down
}

function sshIntoService {
    # Using all profiles, so that we can status of all services
    echo "Listing the running services..."
    docker compose --env-file .env.dev --profile bahmni-lite --profile bahmni-standard --profile bahmni-mart ps

    echo "Enter the SERVICE name which you wish to ssh into:"
    read serviceName
    
    docker compose --env-file .env.dev exec $serviceName /bin/sh
}
...
function showLogsOfService {
    ...
    docker compose --env-file .env.dev logs $serviceName -f
}


function showOpenMRSlogs {
    echo "Opening OpenMRS Logs..."
    docker compose --env-file .env.dev logs openmrs -f 
}

function startMart {
    echo "Starting services with profile 'bahmni-mart'..."
    docker compose --env-file .env.dev --profile bahmni-mart up -d
}
...
function resetAndEraseALLVolumes {
  ...
  if confirm "WARNING: Are you sure you want to DELETE all Bahmni Data and Volumes??"; then
    ...
    docker compose --env-file .env.dev --profile emr --profile bahmni-lite --profile bahmni-standard --profile bahmni-mart down -v
    RESULT=$?
    if [ $RESULT -eq 0 ]; then
...
function restartService {
    ...
    docker compose --env-file .env.dev restart $serviceName
